    ...
    (let [error-response (validate-input talk)]
      (if error-response error-response
        (let [talk-result (communicate-talk-to-ems talk session)]
          ...
          (send-mail (speaker-mail-list talk) (str "Confirmation " (if (exsisting-talk? talk) "on updating" "of") " your JavaZone 2016 submission \"" (talk "title") "\"") (generate-mail-text (slurp (clojure.java.io/resource "speakerMailTemplate.txt"))
...
(defroutes main-routes
  ...
  (POST "/addTalk"  {body :body session :session} (if (submitit-closed?) 
    {:status 403 :body "403 Submitit is closed"} 
    (add-talk (parse-string (slurp body)) session)))
  (GET "/talkJson/:talkid"  request (json-talk ((request :route-params) :talkid)))
  (GET "/needPassword" [] (generate-string {:needPassword (need-submit-password?) :isClosed (submitit-closed?)}))
  (GET "/status" [] (status-page))
  (GET "/uploadPicture" request (upload-picture request))
  (POST "/addPic" request (if (submitit-closed?)  {:status 403 :body "403 Submitit is closed"} (add-picture (mp/multipart-params-request request))))
